...
from transformers import AutoConfig, AutoModelForCausalLM, MistralModel, MistralConfig, MistralForCausalLM
...
from .multimodal_components.mm_align import AlignIndicator, GatedWeightLayer, build_align

from src.utils.constants import *

class AlignGPTMistralConfig(MistralConfig):
    model_type = 'aligngpt_mistral'
    n_indicators = 8
    use_s2 = False
    s2_scales = '336'
    

class AlignGPTMistralForCausalLM(MistralForCausalLM):
    config_class = AlignGPTMistralConfig
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "mm_vision_tower"): # In inference stage
            ...
            self.align_indicators.indicator_embs = build_align(config)
            # print(self.align_indicators.indicator_embs.shape)
    ...
    def initialize_align_components(self):
        ...
        if self.stage == FINETUNE or self.stage == INFERENCE or self.stage == FT_LOCAL:
    ...
    def load_pretrained_weights(self, model_args):
        ...
        if 'finetune' in self.stage or 'inference' in self.stage:
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if self.stage == PRETRAIN:    
                align_indicators = self.align_indicators(align_ids) # [bs, dim_size]
                align_indicators = align_indicators.unsqueeze(dim=1) # [bs, 1, dim_size]
                ...
                if images is not None and input_ids is None:
                    ...
                    if pure_text_embeds is not None:
                        if len(image_embeds) == 0:
                            dummy_al_id = self.align_indicators.mean_align().unsqueeze(dim=1).expand(len(pure_text_embeds), -1, -1)
                            pure_text_embeds = torch.cat([pure_text_embeds, dummy_al_id[:, 0:0, :]], dim=1)
                        # in_pure = True
                        inputs_embeds = torch.cat([inputs_embeds, pure_text_embeds]) if len(inputs_embeds) > 0 else torch.cat([pure_text_embeds])
                        labels = torch.cat([labels, pure_text_labels]) if labels is not None else pure_text_labels
                ...
                if images is not None and input_ids is None:
                    ...
                    if pure_text_embeds is not None:
                        if len(image_embeds) == 0:
                            dummy_scores = self.gated_weight_layer.dummy_forward(len(pure_text_embeds), pure_text_embeds.device, pure_text_embeds.dtype)
                            dummy_al_id = self.align_indicators.lin_comb_local(dummy_scores).unsqueeze(dim=1)
                            pure_text_embeds = torch.cat([pure_text_embeds, dummy_al_id[:, 0:0, :]], dim=1)
                        # in_pure = True
                        inputs_embeds = torch.cat([inputs_embeds, pure_text_embeds]) if len(inputs_embeds) > 0 else torch.cat([pure_text_embeds])
                        labels = torch.cat([labels, pure_text_labels]) if labels is not None else pure_text_labels
                ...
                if images is not None and input_ids is None:
                    ...
                    if pure_text_embeds is not None:
                        if len(image_embeds) == 0:
                            dummy_al_id = self.align_indicators.global_align().unsqueeze(dim=1).expand(len(pure_text_embeds), -1, -1)
                            pure_text_embeds = torch.cat([pure_text_embeds, dummy_al_id[:, 0:0, :]], dim=1)
                        # in_pure = True
                        inputs_embeds = torch.cat([inputs_embeds, pure_text_embeds]) if len(inputs_embeds) > 0 else torch.cat([pure_text_embeds])
                        labels = torch.cat([labels, pure_text_labels]) if labels is not None else pure_text_labels
    ...
    def prepare_inputs_for_generation(self, input_ids, past_key_values=None, inputs_embeds=None, **kwargs):
        ...
        align_ids = kwargs.pop('align_ids', None)
...
AutoConfig.register("aligngpt_mistral", AlignGPTMistralConfig)
AutoModelForCausalLM.register(AlignGPTMistralConfig, AlignGPTMistralForCausalLM)
